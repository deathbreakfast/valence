...
pub async fn run(ctx: &RunContext) -> Result<BenchReport> {
    ...
    for i in 0..depth {
        ...
        Project::create_used(project, valence, valence::use_!("create Project in src/runners/bm_v22.rs; Valence persistence for this feature path; typed store; visible to test harness.")).await?;
    }
    ...
    for _ in 0..ctx.sweep.query_iters.min(50) {
        ...
        let rows = Project::query_used(valence, valence::use_!("query Project in src/runners/bm_v22.rs; Valence persistence for this feature path; typed store; visible to test harness.")).await?;
        last_len = rows.len();
