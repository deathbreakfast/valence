...
pub async fn run(ctx: &RunContext) -> Result<BenchReport> {
    ...
    for i in 0..depth {
        ...
        Project::create_used(project, valence, valence::use_!("create Project in src/runners/bm_v27.rs; Valence persistence for this feature path; typed store; visible to test harness.")).await?;
    }

    clear_privacy_bypass();
    let mut with_filter = Vec::with_capacity(ctx.sweep.query_iters);
    for _ in 0..ctx.warmup {
        let _ = Project::query_used(valence, valence::use_!("query Project in src/runners/bm_v27.rs; Valence persistence for this feature path; typed store; visible to test harness.")).await?;
    }
