//! bm-v29: mixed OLTP (`prod-mix-v1`) for sqlite and hybrid marketing cells.
//!
//! One client, 30s, C=32 on AWS `c6i.xlarge` `--release`: 10% creates, 55%
//! 64-key hot gets, 10% cache-off primary gets, 25% equality filters over a
//! 10k corpus. Isolated v5/v20/v21/v28 studies stay on `marketing-capacity`.
//!
//! ```text
//! cargo run -p valence-bench --release -- run --experiment bm-v29 \
//!   --storage hybrid --duration-secs 30 --concurrency 32 --prefill 10000
//! ```
...
pub async fn run(ctx: &RunContext) -> Result<BenchReport> {
    ...
    report.pass_notes = Some(format!(
        "prod-mix-v1 mixed {:.1} ops/s create_p95 {:.3} hot_p95 {:.3} {cold_note} p95 {:.3} eq_p95 {:.3} error_rate {:.4} (prefill={} cold_keys={})",
...
mod tests {
    ...
    async fn assert_mixed_report(storage: &str) {
        let report = run(&context(storage)).await.expect("bm-v29 report");
    ...
    #[tokio::test]
    async fn bm_v29_mixed_mem_happy() {
        assert_mixed_report("mem").await;
    }

    #[tokio::test]
    async fn bm_v29_mixed_sqlite_happy() {
    ...
    #[tokio::test]
    async fn bm_v29_hybrid_unavailable_skips_sad() {
    ...
    async fn bm_v29_cache_off_primary_semantics() {
        ...
        assert_eq!(report.status, "ok");
        let notes = report.pass_notes.expect("notes");
        assert!(notes.contains("cache-off"), "{notes}");
        let mixed = report.mixed.expect("mixed");
        let hot_share = mixed.hot_get.share;
        let cold_share = mixed.cold_get.share;
        assert!(
            (hot_share - 0.55).abs() <= crate::workload::mixed::MIX_DRIFT_TOLERANCE,
            "hot share {hot_share}"
        );
        assert!(
            (cold_share - 0.10).abs() <= crate::workload::mixed::MIX_DRIFT_TOLERANCE,
            "cold share {cold_share}"
        );
        assert_eq!(cold_ids(128).len(), 64);
        for id in crate::workload::mixed::hot_ids(128) {
            assert!(!cold_ids(128).contains(&id));
        }
    }
}
