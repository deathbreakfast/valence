...
async fn model_crud(session: &mut BootstrapSession, mode: RunMode) -> Result<(), String> {
    ...
    let created = Project::create_used(project, valence, valence::use_!("create Project in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .map_err(|e| e.to_string())?;
    let project_id = created.id().ok_or("missing project id")?.id();
    let fetched = Project::get_used(project_id, valence, valence::use_!("get Project in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .map_err(|e| e.to_string())?;
...
async fn model_update_upsert(session: &mut BootstrapSession, mode: RunMode) -> Result<(), String> {
    ...
    let after_update = Project::update_used(&id, updated, valence, valence::use_!("update Project in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .map_err(|e| e.to_string())?;
    ...
    let after_upsert = Project::upsert_used(&id, upserted, valence, valence::use_!("upsert Project in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .map_err(|e| e.to_string())?;
...
async fn query_filter_eq(session: &mut BootstrapSession, mode: RunMode) -> Result<(), String> {
    ...
    let valence = session.ensure_valence().map_err(|e| e.to_string())?;
    let rows = Project::query_used(valence, valence::use_!("query Project in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
...
async fn typed_field_roundtrip(
    ...
    let valence = session.ensure_valence().map_err(|e| e.to_string())?;
    let created = TypedProbe::create_used(probe, valence, valence::use_!("create TypedProbe in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .map_err(|e| e.to_string())?;
    let id = created
        .id()
        ...
        .id()
        .to_string();
    let fetched = TypedProbe::get_used(&id, valence, valence::use_!("get TypedProbe in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .map_err(|e| e.to_string())?
...
async fn query_filter_datetime(
    ...
    let id = created
        ...
        .id()
        .to_string();
    let rows = TypedProbe::query_used(valence, valence::use_!("query TypedProbe in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
...
async fn query_filter_datetime_miss(
    ...
    let valence = session.ensure_valence().map_err(|e| e.to_string())?;
    TypedProbe::create_used(probe, valence, valence::use_!("create TypedProbe in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .map_err(|e| e.to_string())?;
    let far_future = Utc
...
async fn query_pagination(session: &mut BootstrapSession, mode: RunMode) -> Result<(), String> {
    ...
    let valence = session.ensure_valence().map_err(|e| e.to_string())?;
    let page = Project::query_used(valence, valence::use_!("query Project in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .order_by_name(SortDirection::Asc)
        .limit(2)
        .offset(0)
        .await
...
async fn read_cache_smoke(session: &mut BootstrapSession, mode: RunMode) -> Result<(), String> {
    ...
    let valence = session.ensure_valence().map_err(|e| e.to_string())?;
    let _ = Project::get_used(&id, valence, valence::use_!("get Project in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .map_err(|e| e.to_string())?;
    invalidate("project", &id);
    let again = Project::get_used(&id, valence, valence::use_!("get Project in runner/steps/model.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .map_err(|e| e.to_string())?;
    if again.is_none() {
